...
const BugCard: React.FC<BugCardProps> = ({ bug }) => {
  ...
  const fetchInsight = async () => {
    ...
    setError(null);
    try {
      const data = await getBugInsight(bug);
      setInsight(data);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-4 right-4 bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-xl text-white text-[10px] font-bold uppercase tracking-wider border border-white/20 z-10 pointer-events-none">
            ...
            {!insight ? (
              ...
              >
                ...
                <button
                  ...
                  className="flex items-center gap-2 mx-auto text-slate-400 text-xs font-bold uppercase tracking-widest hover:text-slate-900 transition-colors"
                >
                  Clear Analysis
                </button>
              </motion.div>
